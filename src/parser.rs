...
impl Parser {
    ...
    fn parse_statement(&mut self) -> Statement {
        match self.current_token.token_type {
            TokenType::SelectKwd => self.parse_select_statement(),
    ...
    fn parse_value_expression(&mut self, precedence: i64) -> ValueExpression {
        ...
        while !self.is_peek_token(TokenType::SemiColon)
            && precedence < self.lookup_precedence(self.peek_token.token_type.clone())
        {
    ...
    fn parse_prefix_value_expression(&mut self) -> ValueExpression {
        match self.current_token.token_type {
            ...
            TokenType::NumberType(_) => self.parse_number(),
            TokenType::BoolType(_) => self.parse_bool(),
    ...
    fn parse_infix_expression(&mut self, left: ValueExpression) -> ValueExpression {
        let op = self.lookup_infix(self.current_token.token_type.clone());

        let precedence = self.lookup_precedence(self.current_token.token_type.clone());
        self.next_token();
    ...
    fn parse_number(&mut self) -> ValueExpression {
        match self.current_token.token_type {
            TokenType::NumberType(num) => ValueExpression::Number(num),
            _ => panic!("not expected type!"),
        }
    }
    ...
    fn parse_bool(&mut self) -> ValueExpression {
        match self.current_token.token_type {
            TokenType::BoolType(b) => ValueExpression::Bool(b),
            _ => panic!("not expected type!"),
        }
    }
